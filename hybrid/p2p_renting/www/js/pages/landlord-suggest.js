...
define(['module', 'common'],function(module, common){
    ...
    Func.prototype.load = function(){
        var self = this;
        self.jxhlLocalVars[0].innerHTML = '最佳租客查询中..';
    ...
    Func.prototype.renderData = function(data){
        ...
        for(var i=0; i<data.length; i++){
            var item = data[i];
            $(self.jxhlLocalVars[0]).append('<div data-id="'+data.id+'" data-row="dx_'+i+'">\
            <div>期望住在：'+item.community+', '+
            item.areaRange+'km范围内</div>\
            <div>'+(item.jointRent?'合租':'整租')+', '+
            item.minPrice+'~'+item.maxPrice+'元</div>\
            </div>');
    ...
    Func.prototype.attachEvent = function(row){
        ...
        row.onclick = function(){
            ...
            jxhl.utility.loadJxhlLayout('chat', ['landlord', id]);
        }
    }
